...
func checkWithHTTPOut(w http.ResponseWriter, err error, code int) bool {
	if err != nil {
		log.Fatalln(err)
...
var otherTeamLookup = make(map[bts.Team]*firestore.DocumentRef)
...
func makeTeamLookup(ctx context.Context, fs *firestore.Client) error {
	...
	for {
		...
		team4, err := teamDoc.DataAt("abbreviation")
		if err != nil {
			return err
		}
		...
		for _, n := range otherNames.([]interface{}) {
			otherTeamLookup[bts.Team(n.(string))] = teamDoc.Ref
		}
	}
	return nil
}
...
func btsMCUsage() {
	fmt.Fprintf(flag.CommandLine.Output(), `Usage: btstool [global-flags] bts-mc [flags] [picker [picker ...]]
...
func init() {
	btsMCFlagSet = flag.NewFlagSet("bts-mc", flag.ExitOnError)
...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	if checkWithHTTPOut(w, err, http.StatusBadRequest) {
		return
	}
	...
	if checkWithHTTPOut(w, err, http.StatusInternalServerError) {
		return
	}
	...
	for i, td := range teamDocs {
		...
		teams[i] = bts.Team(team.Abbreviation)
	}
	...
	for _, pickerRef := range pickerRefs {
		...
		for i, teamDoc := range remainingTeamDocs {
			...
			remainingTeams[i] = bts.Team(team.Abbreviation)
		}
